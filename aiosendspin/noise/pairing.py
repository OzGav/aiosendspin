...
from aiosendspin.noise.cpace import CPace, CPaceError, CPaceRole
from aiosendspin.noise.keys import PSK_SIZE, b64url_decode, b64url_encode, psk_id_for
...
if TYPE_CHECKING:
    ...
    from aiosendspin.noise.trust_store import ClientPairingStore, ServerPairingStore
    from aiosendspin.noise.wire import EncryptedWebSocket

# CPace session id and PIN attempt bound.
...
async def run_pairing_psk_server(
    ...
    *,
    ...
    return await _finalize_server(ws, client_id=client_id, store=store)
...
async def run_dynamic_pin_client(
    ...
    *,
    ...
    Returns ``None`` on finalize, else the raw ``server/activate`` leave frame.
    """
    sid = _PAKE_SID_LABEL + handshake_hash
    nonce_b = pin_mod.generate_nonce()
    try:
        async with asyncio.timeout(_ATTEMPT_TIMEOUT_S):
            await ws.send_str(
                ClientPairInitMessage(
                    payload=ClientPairInitPayload(commit_B=b64url_encode(pin_mod.commit(nonce_b))),
                ).to_json(),
            )
            ...
            cpace = CPace.start(role=CPaceRole.RESPONDER, prs=pin.encode("ascii"), sid=sid)
            ...
            peer_share = _decode_field(auth.payload.pake_msg_1, "pake_msg_1")
            try:
                cpace.derive(peer_share)
            except CPaceError:
                await abort_pairing(ws, PairAbortReason.PIN_MISMATCH)

            confirm = await _receive_pairing(ws, ServerPairConfirmMessage)
            if not cpace.verify(_decode_field(confirm.payload.server_kc, "server_kc")):
...
async def run_dynamic_pin_server(
    ...
    *,
    ...
    Returns the persisted record, or ``None`` when ``finalize`` is false (left pairing).
    """
    sid = _PAKE_SID_LABEL + handshake_hash
    ...
    try:
        async with asyncio.timeout(_ATTEMPT_TIMEOUT_S):
            ...
            cpace = CPace.start(role=CPaceRole.INITIATOR, prs=pin.encode("ascii"), sid=sid)
            await ws.send_str(
                ServerPairAuthMessage(
            ...
            peer_share = _decode_field(auth.payload.pake_msg_2, "pake_msg_2")
            try:
                cpace.derive(peer_share)
            except CPaceError:
                await abort_pairing(ws, PairAbortReason.PIN_MISMATCH)
            ...
            nonce_b = _decode_field(
                confirm.payload.nonce_B, "nonce_B", expect_len=pin_mod.NONCE_SIZE
            )
            if (
                not cpace.verify(_decode_field(confirm.payload.client_kc, "client_kc"))
                or not pin_mod.verify_commit(nonce_b, commit_b)
                or pin_mod.derive_pin(handshake_hash, nonce_a, nonce_b, pin_length) != pin
            ):
                await abort_pairing(ws, PairAbortReason.PIN_MISMATCH)
            if finalize:
                return await _finalize_server(ws, client_id=client_id, store=store)
            await _receive_pairing(ws, ClientPairFinalizeMessage)  # drain; left pairing, no record
            return None
    except TimeoutError:
        await abort_pairing(ws, PairAbortReason.ATTEMPT_TIMEOUT)
...
async def run_static_pin_client(
    ...
    *,
    ...
    try:
        async with asyncio.timeout(_ATTEMPT_TIMEOUT_S):
            await ws.send_str(ClientPairInitMessage(payload=ClientPairInitPayload()).to_json())

            cpace = CPace.start(role=CPaceRole.RESPONDER, prs=static_pin.encode("ascii"), sid=sid)
            auth = await _receive_pairing(ws, ServerPairAuthMessage)
            await ws.send_str(
                ClientPairAuthMessage(
                    payload=ClientPairAuthPayload(pake_msg_2=b64url_encode(cpace.public_share)),
...
async def run_static_pin_server(
    ...
    *,
    ...
    """
    sid = _PAKE_SID_LABEL + handshake_hash
    await _receive_pairing(ws, ClientPairInitMessage)
    try:
        async with asyncio.timeout(_ATTEMPT_TIMEOUT_S):
            pin = await pin_provider()
            cpace = CPace.start(role=CPaceRole.INITIATOR, prs=pin.encode("ascii"), sid=sid)
            ...
            confirm = await _receive_pairing(ws, ClientPairConfirmMessage)
            if not cpace.verify(_decode_field(confirm.payload.client_kc, "client_kc")):
                await abort_pairing(ws, PairAbortReason.PIN_MISMATCH)
            if finalize:
                return await _finalize_server(ws, client_id=client_id, store=store)
            await _receive_pairing(ws, ClientPairFinalizeMessage)  # drain; left pairing, no record
...
async def _finalize_server(
    ...
    *,
    ...
    """Receive ``client/pair-finalize``, persist the record, ack with ``server/pair-finalize``."""
    finalize = await _receive_pairing(ws, ClientPairFinalizeMessage)
    psk = _decode_field(finalize.payload.long_term_psk, "long_term_psk", expect_len=PSK_SIZE)
    record = ServerPairingRecord(psk_id=psk_id_for(psk), psk=psk, client_id=client_id)
