...
class SendspinConnection:
    ...
    async def _exchange_hellos(self) -> bool:
        ...
        legacy = self._pending_first_text is not None
    ...
    def _playback_capable(self) -> bool:
        ...
        if self._noise_psk.category is PskCategory.SENTINEL:
            return (
                self._server.unpaired_access_enabled and self._client_info.unpaired_access.enabled
            )
        return False
    ...
    async def initiate_pairing(self, attempt: PairingAttempt) -> None:
        """Run an operator-initiated pairing on this connection.

        On failure the connection is left deactivated; the caller must disconnect it.
        """
        if self._pairing_attempt is not None:
        ...
        try:
            ...
            if not await self._pair(dispatched):
    ...
    async def _pair(self, transport: EncryptedWebSocket) -> bool:
        ...
        return await self._rehandshake_to(transport, record.as_resolved())
    ...
    async def _run_pairing_protocol(
        ...
        assert self._noise_psk is not None
        # Over a long-term PSK the pair already exists: verify and leave pairing, don't finalize.
        finalize = self._noise_psk.category is not PskCategory.LONG_TERM
        ...
        return await run_dynamic_pin_server(
            ...
            handshake_hash=self._handshake_hash,
            pin_provider=self._pairing_attempt.pin_provider,
            pin_length=await self._negotiated_dynamic_pin_length(transport),
            client_id=self._client_id,
            store=self._server.pairing_store,
            finalize=finalize,
        )

    async def _negotiated_dynamic_pin_length(self, transport: EncryptedWebSocket) -> int:
        ...
        if client_min is None or not MIN_PIN_DIGITS <= client_min <= MAX_PIN_DIGITS:
            await abort_pairing(transport, PairAbortReason.METHOD_NOT_SUPPORTED)
