...
from aiosendspin.noise.models import NoiseHandshakeMessage, PairAbortMessage, PairAbortPayload
...
# Static-PIN pairing-window lifetime: how long the client waits for the operator gesture
# before closing the attempt silently.
WINDOW_LIFETIME_S: float = 300.0

# PIN-pairing method families, subject to lockout and the locked_out descriptor.
_PIN_METHODS: frozenset[PairMethod] = frozenset({PairMethod.DYNAMIC_PIN, PairMethod.STATIC_PIN})
...
class SendspinConnection:
    ...
    async def _run_pairing_protocol(self) -> str | None:
        ...
        if method in _PIN_METHODS and await store.is_pin_locked_out(method):
            await self._abort_pairing(PairAbortReason.LOCKED_OUT)
        if method is PairMethod.PAIRING_PSK:
            leftover = await run_pairing_psk_client(
                self._ws, server_id=self._server_id, store=store
            )
        elif method is PairMethod.STATIC_PIN:
            ...
            async with asyncio.timeout(WINDOW_LIFETIME_S):
                await self._client.pairing_window()
            leftover = await run_static_pin_client(
                self._ws,
        ...
        else:  # PairMethod.DYNAMIC_PIN
            assert self._handshake_hash is not None
            try:
                leftover = await run_dynamic_pin_client(
                    self._ws,
                    handshake_hash=self._handshake_hash,
    ...
    async def _supported_pair_methods(self) -> tuple[PairMethod, ...]:
        ...
        ``dynamic_pin`` requires an out-channel (``pin_display``); ``static_pin`` requires a
        pairing-window gesture (``pairing_window``) and a configured PIN.
        """
        ...
        if (
            config.static_pin_enabled
            and self._client.pairing_window is not None
            and await self._client.pairing_store.static_pin() is not None
        ):
            methods.append(PairMethod.STATIC_PIN)
        if config.dynamic_pin_enabled and self._client.pin_display is not None:
    ...
    async def _handle_management_request(self, message: _ManagementRequest) -> None:
        ...
        match message:
            ...
            case ManagementGetPairingConfigMessage():
                payload, effect = await handle_get_pairing_config(store)
            case ManagementSetPairingConfigMessage(payload=request):
                payload, effect = await handle_set_pairing_config(store, request)
            case _:
