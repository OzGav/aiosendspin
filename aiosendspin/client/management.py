...
# Pairing methods this client implements a flow for.
IMPLEMENTED_PAIR_METHODS = frozenset(
    {PairMethod.PAIRING_PSK, PairMethod.DYNAMIC_PIN, PairMethod.STATIC_PIN}
)
...
async def handle_get_pairing_config(
    store: ClientPairingStore,
    *,
    implemented: frozenset[PairMethod] = IMPLEMENTED_PAIR_METHODS,
) -> tuple[ManagementResultPayload, ManagementEffect]:
    ...
    data = ManagementResultData(
        ...
        static_pin=(
            ...
            if PairMethod.STATIC_PIN in implemented
            else None
        ),
        dynamic_pin=(
            await _method_config(
            ...
            if PairMethod.DYNAMIC_PIN in implemented
            else None
        ),
...
async def handle_set_pairing_config(
    ...
    *,
    ...
    if any(cfg is not None and method not in implemented for method, cfg in methods):
        return _result(ManagementResult.INVALID), ManagementEffect.NONE
...
def _valid_static_pin(pin: str) -> bool:
    """Return whether ``pin`` is exactly 8 decimal digits."""
    return len(pin) == _STATIC_PIN_DIGITS and pin.isdigit()
