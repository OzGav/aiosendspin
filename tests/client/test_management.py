...
_WITH_STATIC_PIN = frozenset(
    {PairMethod.PAIRING_PSK, PairMethod.STATIC_PIN, PairMethod.DYNAMIC_PIN}
)
...
class _ExhaustedClientStore(InMemoryClientPairingStore):
    """Store that cannot persist further records."""

    async def can_store_record(self) -> bool:
        return False


class _BoundedClientStore(InMemoryClientPairingStore):
    """Store that reports fixed storage accounting (one full record per remaining slot)."""

    async def storage_accounting(self) -> StorageReport:
        used = len(await self.records())
...
async def test_get_pairing_config_projects_state() -> None:
    ...
    payload, effect = await handle_get_pairing_config(store, implemented=_WITHOUT_STATIC_PIN)
    assert effect is ManagementEffect.NONE
    data = payload.data
...
async def test_get_pairing_config_shows_static_pin_when_implemented() -> None:
    ...
    store = InMemoryClientPairingStore()
    payload, _ = await handle_get_pairing_config(store, implemented=_WITH_STATIC_PIN)
    assert payload.data is not None
...
async def test_set_pairing_config_toggles_enabled() -> None:
    ...
    payload, effect = await handle_set_pairing_config(
        store, ManagementSetPairingConfigPayload(pairing_psk=SetPairingPskConfig(enabled=False))
    )
    assert payload.result is ManagementResult.OK
    assert effect is ManagementEffect.NONE
...
async def test_set_pairing_config_invalid_psk() -> None:
    ...
    store = InMemoryClientPairingStore()
    payload, _ = await handle_set_pairing_config(
        store, ManagementSetPairingConfigPayload(pairing_psk=SetPairingPskConfig(psk="too-short"))
    )
    assert payload.result is ManagementResult.INVALID
...
async def test_set_pairing_config_sets_min_pin_length() -> None:
    ...
    store = InMemoryClientPairingStore()
    payload, _ = await handle_set_pairing_config(
        store, ManagementSetPairingConfigPayload(dynamic_pin=SetDynamicPinConfig(min_pin_length=8))
    )
    assert payload.result is ManagementResult.OK
...
async def test_set_pairing_config_invalid_min_pin_length() -> None:
    ...
    store = InMemoryClientPairingStore()
    payload, _ = await handle_set_pairing_config(
        store, ManagementSetPairingConfigPayload(dynamic_pin=SetDynamicPinConfig(min_pin_length=3))
    )
    assert payload.result is ManagementResult.INVALID
...
async def test_set_pairing_config_stores_static_pin() -> None:
    ...
    store = InMemoryClientPairingStore()
    payload, _ = await handle_set_pairing_config(
        store, ManagementSetPairingConfigPayload(static_pin=SetStaticPinConfig(pin="12345678"))
    )
    assert payload.result is ManagementResult.OK
...
async def test_set_pairing_config_invalid_static_pin() -> None:
    """A non-8-digit pin is rejected and nothing is stored."""
    store = InMemoryClientPairingStore()
    payload, _ = await handle_set_pairing_config(
        store, ManagementSetPairingConfigPayload(static_pin=SetStaticPinConfig(pin="12ab"))
    )
    assert payload.result is ManagementResult.INVALID
...
async def test_get_pairing_config_omits_static_pin_secret() -> None:
    ...
    payload, _ = await handle_get_pairing_config(store)
    assert payload.data is not None
    assert payload.data.static_pin is not None
...
async def test_set_pairing_config_unimplemented_method_is_invalid() -> None:
    ...
    payload, _ = await handle_set_pairing_config(
        ...
        implemented=_WITHOUT_STATIC_PIN,
    )
    assert payload.result is ManagementResult.INVALID
...
async def test_set_pairing_config_clears_lockout() -> None:
    ...
    payload, _ = await handle_set_pairing_config(
        store, ManagementSetPairingConfigPayload(dynamic_pin=SetDynamicPinConfig(locked_out=False))
    )
    assert payload.result is ManagementResult.OK
...
async def test_set_pairing_config_rejects_lockout_true() -> None:
    ...
    store = InMemoryClientPairingStore()
    payload, _ = await handle_set_pairing_config(
        store, ManagementSetPairingConfigPayload(dynamic_pin=SetDynamicPinConfig(locked_out=True))
    )
    assert payload.result is ManagementResult.INVALID
