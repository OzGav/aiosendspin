...
async def test_inflight_pairing_not_displaced_by_pairing() -> None:
    """An in-flight pairing is protected from a competing incoming pairing."""
    client = _client()
    client._admitted_connection = _FakeConnection(  # type: ignore[assignment]
...
async def test_inflight_pairing_not_displaced_by_playback() -> None:
    """An in-flight pairing is protected from an incoming playback connection."""
    client = _client()
    client._admitted_connection = _FakeConnection(  # type: ignore[assignment]
        activities=[Activity.PAIRING]
    )
...
async def test_inflight_pairing_displaced_by_management() -> None:
    """Management still outranks an in-flight pairing."""
    client = _client()
    client._admitted_connection = _FakeConnection(  # type: ignore[assignment]
        activities=[Activity.PAIRING]
    )
