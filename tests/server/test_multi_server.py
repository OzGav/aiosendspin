...
class TestEncryptedActivities:
    ...
    @pytest.mark.asyncio
    async def test_sentinel_without_unpaired_access_declares_no_roles(
        self, mock_server: _MockServer
    ) -> None:
        """A Sentinel PSK without unpaired access activates no roles (server toggle off)."""
        conn = SendspinConnection(mock_server, wsock_client=AsyncMock())
    ...
    @pytest.mark.asyncio
    async def test_sentinel_with_unpaired_access_activates_roles(
        self, mock_server: _MockServer
    ) -> None:
        """A Sentinel PSK activates roles only when both server and client allow unpaired access."""
        mock_server.unpaired_access_enabled = True
        conn = SendspinConnection(mock_server, wsock_client=AsyncMock())
        fake = await _exchange_hellos_encrypted(
            conn, category=PskCategory.SENTINEL, unpaired_access=True
        )
    ...
    @pytest.mark.asyncio
    async def test_sentinel_server_toggle_off_overrides_client(
        self, mock_server: _MockServer
    ) -> None:
        """Client admitting unpaired access activates no roles when the server toggle is off."""
        mock_server.unpaired_access_enabled = False
        conn = SendspinConnection(mock_server, wsock_client=AsyncMock())
        fake = await _exchange_hellos_encrypted(
            conn, category=PskCategory.SENTINEL, unpaired_access=True
        )
