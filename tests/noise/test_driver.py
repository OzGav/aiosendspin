...
class _PairedFakeWS:
    """In-memory bidirectional WS satisfying :class:`HandshakeWebSocket`.

    Two instances share a pair of queues — one's outbound is the other's
    inbound — so the driver's two roles can run against each other via
    :func:`asyncio.gather`.
    """
    ...
    async def send_str(self, data: str) -> None:
        await self._send_q.put(WSMessage(WSMsgType.TEXT, data, ""))
    ...
    async def close_outbound(self) -> None:
        """Signal end-of-stream to the peer (yields a CLOSED frame on receive)."""
        await self._send_q.put(None)


def _make_pair() -> tuple[_PairedFakeWS, _PairedFakeWS]:
    a_to_b: asyncio.Queue[WSMessage | None] = asyncio.Queue()
    b_to_a: asyncio.Queue[WSMessage | None] = asyncio.Queue()
    return _PairedFakeWS(a_to_b, b_to_a), _PairedFakeWS(b_to_a, a_to_b)
...
async def test_full_handshake_yields_paired_encrypted_websockets(
    ...
    server_ws, client_ws = _make_pair()
...
async def test_handshake_timeout_aborts() -> None:
    ...
    server_id = Identity.generate()
    server_ws, _client_ws = _make_pair()
